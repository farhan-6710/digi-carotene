-- Migration 011 — auto-link profiles when email matches team_members or clients.email.
-- Triggers only; no app RPCs. Team member email takes precedence over client.
...
-- Link existing auth users whose email already matches roster (one-time on apply).
do $$
declare
  auth_email text;
begin
